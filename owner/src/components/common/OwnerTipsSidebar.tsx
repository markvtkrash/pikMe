...
// in this app — shown underneath each tip card for visual variety.
const FOOD_IMAGES = [
...
// One flat list — left and right panels show two different tips at once
...
export function OwnerTipsSidebar() {
  ...
  // Re-rolled whenever the route changes (this layout persists across
  // in-app navigation, so a plain one-time useState wouldn't reflect a new
  // "page load") and held steady for as long as the owner stays on that
  // page — no auto-rotation out from under them while they're reading it.
  const index = useMemo(() => Math.floor(Math.random() * TIPS.length), [pathname]);
