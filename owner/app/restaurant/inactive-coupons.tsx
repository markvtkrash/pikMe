...
// Deactivated but still within its expiry window — the direct complement of
// Active Coupons. A coupon that's both inactive AND expired shows up in
// Expired Coupons instead, not here.
export default function InactiveCouponsScreen() {
  ...
  async function loadInactiveCoupons() {
    if (!restaurant) {
    ...
    try {
      ...
      const now = new Date();
      const inactive = (allCoupons as Coupon[]).filter((c) => !c.is_active && new Date(c.expiry_date) > now);
      setCoupons(inactive);
    } catch (error: any) {
      console.error('[inactive-coupons] Load error:', error);
      Alert.alert('Error', 'Failed to load inactive coupons');
    } finally {
      setLoading(false);
    }
  }
  ...
  async function handleReactivate(couponId: string) {
    setReactivatingId(couponId);
    try {
      await updateCoupon(couponId, { isActive: true });
      setCoupons((prev) => prev.filter((c) => c.id !== couponId));
    } catch (error: any) {
      console.error('[inactive-coupons] Reactivate error:', error);
  ...
  return (
    ...
    <View style={styles.pageWrapper}>
      <View style={styles.header}>
        ...
        <View style={{ flex: 1 }}>
          <Text style={styles.title}>Inactive Coupons</Text>
          <Text style={styles.count}>{coupons.length} inactive</Text>
        </View>
      </View>
      ...
      {coupons.length === 0 ? (
        <View style={styles.emptyContainer}>
          ...
          <Text style={styles.emptyText}>No inactive coupons</Text>
          <Text style={styles.emptySubtext}>Deactivated coupons that haven't expired yet will show up here</Text>
        </View>
      ) : (
        <FlatList
          data={coupons}
          ...
          renderItem={({ item }) => (
            <View style={styles.couponCard}>
              ...
              <View style={styles.couponFooter}>
                <TouchableOpacity
                  style={[styles.reactivateBtn, reactivatingId === item.id && styles.buttonDisabled]}
...
const styles = StyleSheet.create({
  ...
  inactiveBadgeContainer: { backgroundColor: '#757575', paddingVertical: 8, paddingHorizontal: 12, borderRadius: 8, alignSelf: 'flex-start' },
  inactiveBadge: { fontSize: 13, fontWeight: '900', color: '#fff', letterSpacing: 0.5 },
  ...
  itemBadge: { fontSize: 10, fontWeight: '700', color: '#fff', backgroundColor: '#757575', paddingHorizontal: 6, paddingVertical: 2, borderRadius: 4 },
  couponRow: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 4, flexWrap: 'wrap', gap: 6 },
  discountValue: { fontSize: 18, fontWeight: '800', color: '#555' },
